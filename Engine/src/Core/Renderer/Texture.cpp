...
namespace ARB {
	Texture::Texture(std::string& fullPath, bool flip) {
		...
		std::string name = fullPath.substr(fullPath.find_last_of("/"), fullPath.length());
		texture->name = name;
		...
		if (texture->data)
			textureLogger->logger->error("Loaded texture {0} from {1}", name, fullPath.substr(0, fullPath.find_last_of("/") + 1));
		else {
			textureLogger->logger->error("Unable to load texture {0} from {1}", name, fullPath.substr(0, fullPath.find_last_of("/") + 1));
			texture->data = stbi_load("default_textures", &texture->width, &texture->height, &texture->nrChannels, 0);
		}
		stbi_set_flip_vertically_on_load(!flip);
	}
